...
| `PVENICHang` (Loki) | Kernel logged "Detected Hardware Unit Hang" — the e1000e TX ring wedge. | Full detail in `docs/runbooks/pve-nic-hang.md`. Host is alive but off the network; reboot is the only recovery. Threshold is >10 hangs in 10m. A real wedge produced 4,658 events in 2h35m (1,800/hr) with **zero** `Reset adapter` attempts and the host dark the whole time; mitigated hosts show zero hangs at all when counted with `journalctl -k`. |
